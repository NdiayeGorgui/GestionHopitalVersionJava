...
@Service
public class AdmissionService2 {
	...
	/*public static Predicate<Patient> paatientAMoinsDeSeizeAns(Admission a) {

		return p -> p.getDateNaissance() != null
				&& ChronoUnit.YEARS.between(p.getDateNaissance(), a.getDateAdmission()) <= 16;
	}*/
		...
		if (!a.getPatient().getAssurance().getNomAssurance().equalsIgnoreCase(Constante.RAMQ)) {
			...
			if (a.getLit().getChambre().getDescription().equalsIgnoreCase(Constante.PRIVE)) {
				if (a.isTelephoneDisponible() == true && a.isTeleviseurDisponible() == true) {
					valeur = Constante.TARIF_PRIVE + Constante.TARIF_TELEPHONE + Constante.TARIF_TELEVISION;
				} else if (a.isTelephoneDisponible() == true && a.isTeleviseurDisponible() == false) {
					valeur = Constante.TARIF_PRIVE + Constante.TARIF_TELEPHONE;
				} else if (a.isTelephoneDisponible() == false && a.isTeleviseurDisponible() == true) {
					valeur = Constante.TARIF_PRIVE + Constante.TARIF_TELEVISION;
				} else {
					valeur = Constante.TARIF_PRIVE;
				}
				...
				if (a.isTelephoneDisponible() == true && a.isTeleviseurDisponible() == true) {
					valeur = Constante.TARIF_SEMI_PRIVE + Constante.TARIF_TELEPHONE + Constante.TARIF_TELEVISION;
				} else if (a.isTelephoneDisponible() == true && a.isTeleviseurDisponible() == false) {
					valeur = Constante.TARIF_SEMI_PRIVE + Constante.TARIF_TELEPHONE;
				} else if (a.isTelephoneDisponible() == false && a.isTeleviseurDisponible() == true) {
					valeur = Constante.TARIF_SEMI_PRIVE + Constante.TARIF_TELEVISION;
				} else {
					valeur = Constante.TARIF_SEMI_PRIVE;
				}
			}
			// si le patient choisit une chambre (lit) Standard
			else {
				if (a.isTelephoneDisponible() == true && a.isTeleviseurDisponible() == true) {
					valeur = Constante.TARIF_TELEPHONE + Constante.TARIF_TELEVISION;
				} else if (a.isTelephoneDisponible() == true && a.isTeleviseurDisponible() == false) {
					valeur = Constante.TARIF_TELEPHONE;
				} else if (a.isTelephoneDisponible() == false && a.isTeleviseurDisponible() == true) {
					valeur = Constante.TARIF_TELEVISION;
				}
			}
		}
	...
	public void calculFacturation(Admission a, double tarifTelevision, double tarifChambre, double valeur) {
		if (a.isTelephoneDisponible() == true && a.isTeleviseurDisponible() == true) {
